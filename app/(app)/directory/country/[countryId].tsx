...
export default function OfflineCountryDirectoryScreen() {
  const router = useRouter();
  const { countryId: countryIdParam } = useLocalSearchParams<{ countryId?: string }>();
  const { t, i18n } = useTranslation();
  const countryId =
    typeof countryIdParam === 'string' && isOfflineCountryId(countryIdParam)
      ? countryIdParam
      : null;
  const country = countryId ? getOfflineCountry(countryId) : undefined;
  const cities = useMemo(
    () => countryId ? getOfflineCities(countryId) : [],
    [countryId]
  );
  const [selectedCityId, setSelectedCityId] = useState<OfflineCityId | null>(null);
  const [query, setQuery] = useState('');
  const [category, setCategory] = useState<CategoryFilter>('all');
  const cityId = cities.some((city) => city.id === selectedCityId)
    ? selectedCityId
    : cities[0]?.id ?? null;
    ...
    return (
      <SafeAreaView style={styles.safeArea}>
        <View style={styles.emptyState}>
          <Text style={styles.emptyTitle}>{t('directory.countryNotFound')}</Text>
          <TouchableOpacity
            style={styles.returnButton}
            onPress={() => router.replace('/(app)/directory')}
            accessibilityRole="button"
          >
            <Text style={styles.returnButtonText}>{t('directory.backCountries')}</Text>
          </TouchableOpacity>
        </View>
      </SafeAreaView>
    );
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        <TouchableOpacity
          ...
          accessibilityRole="button"
          accessibilityLabel={t('directory.backCountries')}
        >
          <Text style={styles.backButtonText}>‹</Text>
        </TouchableOpacity>
        <View style={styles.headerText}>
          <Text style={styles.title}>
            {country.flagEmoji} {getLocalizedText(country.name, i18n.language)}
          </Text>
